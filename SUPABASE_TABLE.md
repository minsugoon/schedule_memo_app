...
| `memo`         | text        | ✅       | —                   | 메모 내용 (40자 이내, 애플리케이션 레벨 검증) |
...
### 2026-07-14 업데이트 (프론트엔드 검증 규칙 변경)

| 항목                | 기존 문서       | 실제 코드 (이번 업데이트)                                        |
| ------------------- | --------------- | ----------------------------------------------------------------- |
| `schedules.memo` 최대 길이 | 50자 이내 | ✅ 40자 이내로 축소 (`InputSection`/`ItemCard`/`MemoView` 공통 검증) |

> DB 컬럼 자체(`memo text`)에는 길이 제약이 없으며, 40자 제한은 애플리케이션 레벨(입력 `maxLength` + 저장 전 유효성 검사)에서만 강제된다. 스키마 변경은 없음.
