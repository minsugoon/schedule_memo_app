...
| 컴포넌트 | 역할 |
|----------|------|
| `ScheduleApp` | 전체 상태(탭, 뷰 모드, 테마, 모달) 관리, `useAuth`/`useSchedules`/`useTabs` 연결, DB row → `ScheduleItem` 변환 |
| `AppHeader` | 실시간 시계, 오늘 날짜, 테마 토글, 새로고침, 로그아웃 버튼(조건부) |
| `TabBar` | 사용자 정의 탭(최대 5개) 전환, 탭별 개수 표시, 탭 추가/이름변경/삭제, 완료 보기·메모 뷰 토글 |
| `InputSection` | 시작/종료 날짜·시간, 메모 입력, Enter 키 이동, 50자 글자 수 카운터, 도움말 버튼 |
| `ItemList` | 탭/완료여부 기준 필터링 → `sortItems()` 정렬 → `ItemCard` 렌더링 |
| `ItemCard` | 완료 토글, 인라인 수정(날짜·시간·메모), 삭제, 저장 시 탭 재배정(`TabSelectModal`), 오늘/진행중/탭 뱃지, 클릭으로 아이콘 오버레이 또는 메모 내용 펼치기·접기 전환(잘림 자동 감지) |
| `MemoView` | 날짜 없는 메모(`memo` 탭) 전용 목록 + 빠른 추가 입력, `ItemCard` 재사용 |
...
- **카드 내용 펼치기/접기** — 메모가 말줄임 처리된 카드를 클릭하면 전체 내용 펼침, 펼친 상태에서는 수정/삭제 버튼이 뱃지 자리(우측)로, 뱃지는 메모 하단으로 이동. 말줄임 없는 카드는 기존 아이콘 오버레이 토글 동작 유지
- **긴 메모 오버플로우 방지** — `.item-body-col`/`.item-lines`에 `min-width: 0` + `overflow: hidden`, `.item-memo-line`에 `max-width: 100%` 적용해 50자 메모도 카드 폭 안에서 처리
- **"오늘"/"진행중" 자동 뱃지** — `getBadgeInfo` 기반 표시
- **완료 항목 보기 토글** — 탭 바에서 완료 항목 표시/숨김
