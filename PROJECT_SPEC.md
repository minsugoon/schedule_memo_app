...
| `ItemCard` | 완료 토글, 인라인 수정(날짜·시간·메모), 삭제, 저장 시 탭 재배정(`TabSelectModal`), 오늘/진행중/탭 뱃지, 클릭 시 `isContentExpanded` 토글로 메모 전체 펼치기·접기(펼친 상태는 메모 옆에 수정/삭제 버튼, 뱃지는 메모 하단으로 이동) |
...
- **카드 내용 펼치기/접기** — 카드를 클릭하면 `isContentExpanded` 상태가 토글되며 메모 전체 내용이 펼쳐짐(잘림 자동 감지 로직 없이 클릭 = 토글). 펼친 상태에서는 메모 옆 자리에 수정/삭제 버튼(`.card-action-inline`)이 나타나고 오늘/진행중/탭 뱃지는 메모 하단(`.item-badge-bottom`)으로 이동, 재클릭하면 기본 상태로 복귀. 구 아이콘 오버레이(`.item-icons`) 방식은 완전히 제거됨
