...
│   │   ├── TabMoveModal.tsx      # 메모에 날짜 추가 시 이동할 탭 선택 모달
│   │   ├── TabSelectModal.tsx    # 카드 수정 저장 시 탭 선택 모달
...
| `ItemCard` | 완료 토글, 인라인 수정(날짜·시간·메모), 삭제, 저장 시 탭 재배정(`TabSelectModal`), 오늘/진행중/탭 뱃지 |
| `MemoView` | 날짜 없는 메모(`memo` 탭) 전용 목록 + 빠른 추가 입력, `ItemCard` 재사용 |
| `TabNameModal` | 탭 추가/이름 수정(최대 2자, 추천 이름 칩), 기본 탭이 아니면 삭제 가능 |
| `TabMoveModal` | 메모에 날짜가 추가될 때 이동할 실제 탭 선택 |
| `TabSelectModal` | 카드 편집 저장 시 목적지 탭 선택 (all/memo 제외) |
...
| `fmtShort(date)` | `06월 09일(금)` 형식 포맷 + 요일 타입(`sat`/`sun`/`normal`) |
...
- **메모 전용 뷰** — 날짜 없는 메모를 별도 `memo` 탭에서 관리, 날짜 추가 시 실제 탭으로 이동(`TabMoveModal`)
